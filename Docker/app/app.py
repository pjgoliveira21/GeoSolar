...
@app.route('/map/')
def map_page():
    return render_template('map/map.html', tL = request.args.get('targetLatitude', None), tLg = request.args.get('targetLongitude', None))
...
def get_stations_route():
    ...
    stations = get_stations(offset=offset, limit=limit, search=search)
